...
\begin{itemize}
    ...
    \item We analyze deployment friendliness through operator portability, model size, measured latency bottlenecks, and controllable CP iteration budgets.
    \item We show that at this scale single-core sequential inference is already real time, and that the dominant deployment cost is the isolated CP extraction kernel rather than the S-AFF head.
...
\section{Architectural Insight: Capacity as a Runtime-Addressable Representation}
\label{sec:insight}

Most efficient-HPE work shrinks a network: fewer layers, narrower channels, quantized weights. \method\ rests on a different and, we argue, more useful observation for edge sensing: \emph{decomposition-first inference separates the expensive signal-organization kernel from the learned pose predictor.} The deployment question is no longer a black-box neural-network compression problem; it becomes a measured pipeline problem with a tiny portable head and one isolated factorization bottleneck. Three consequences follow, and they---not raw parameter count---are what make the system deployable on tiny hardware.

\textbf{(1) The learned predictor becomes tiny and portable.}
S-AFF consumes compact CP factors using dense layers, one-dimensional convolutions, pooling, and softmax gates. These are standard neural operators supported by ONNX Runtime, TorchScript, TensorRT, and CPU vector libraries. The deployable head is therefore small, regular, and easy to export, unlike large direct-HPE backbones or branch-heavy tree ensembles.

\textbf{(2) The learned cost is isolated and predictable.}
Because signal organization is factored out of the network, the \emph{learned} predictor is tiny and the \emph{fixed} decomposition dominates cost. On a development-workstation single core we measure the S-AFF forward pass at $\approx0.04$\,ms per frame versus $\approx0.5$--$13$\,ms for CP-ALS across rank $2$--$8$ and $5$--$35$ iterations---a $27$--$350\times$ gap (Sec.~\ref{sec:parallel}). The system's compute profile thus collapses onto a \emph{single analyzable kernel} whose cost is linear and predictable in rank and iterations. Predictable compute is a systems virtue in itself, and it tells us exactly where to optimize: warm-started or amortized (learned feed-forward) factor extraction, not the neural head.

\textbf{(3) Independence does not need parallelism to be useful.}
The mode-specialized branches share no weights or activations and are formally parallel, but at this scale that independence does \emph{not} translate into a wall-clock win: single-core sequential inference is already real time, fork--join overhead exceeds the sub-millisecond branch compute, and BLAS multithreading does not shrink either stage on these small tensors (Sec.~\ref{sec:parallel}). Claiming ``we run in parallel, therefore we are fast'' would be false here; claiming ``we need neither a GPU, nor threads, nor a second core to hit real time'' is both true and a stronger deployability statement.

Together these give the paper's organizing principle: in decomposition-first sensing, \emph{representation design determines deployment cost}. The neural head is already cheap; the remaining hardware problem is the CP kernel, whose rank and iteration count provide measurable latency knobs.
...
The decomposition stage is not restricted to CP. A complete system could use PCA, SVD, matrix NMF, Tucker decomposition, ICA, sparse coding, or a learned autoencoder. We choose CP for this version because it gives the best trade-off among downstream accuracy, modal interpretability, compactness, and deployability.

PCA/SVD are fast and stable, but the resulting factors can be negative and are harder to map back to physical wireless energy. Matrix NMF keeps non-negativity, but it must flatten $X$ into a matrix; after flattening, link, subcarrier, and packet axes are mixed. Tucker decomposition is more expressive, but the core tensor creates interactions that are harder to explain and heavier to deploy. Autoencoders can learn powerful features, but they move the decomposition back into a black-box neural model, weakening the main claim of the paper. CP is not the only possible decomposition, and PCA can be a strong purely predictive baseline. However, CP is the cleanest system point: it preserves all three CSI modes, produces compact factors, keeps the representation inspectable, and enables component-aware fusion and hard routing.
...
The independence of the mode-specialized branches invites a parallel implementation, but we lead with the honest measured conclusion: \emph{at this model scale, parallelism is not where the win is.} On a development-workstation single core, a sequential pass over CP+S-AFF is already real time, and the learned predictor is one to two orders of magnitude cheaper than decomposition: the S-AFF forward pass measures $\approx0.04$\,ms per frame, versus $\approx0.5$--$13$\,ms for CP-ALS across rank $2$--$8$ at $5$--$35$ iterations, and BLAS multithreading reduces neither at these tensor sizes. Consistent with this, an on-device fork--join of the encoder branches ran \emph{slower} than the single-thread sequential path ($0.70\times$). We therefore treat the fork--join structure below as a \emph{latent} property that becomes useful only if CP is separately accelerated, and we bound its benefit rather than claim it.

\textbf{Where the independence actually is.}
After CP produces the three factor matrices $A\in\mathbb{R}^{L\times R}$, $B\in\mathbb{R}^{S\times R}$, $C\in\mathbb{R}^{P\times R}$, the three mode-specialized encoders read \emph{disjoint} slices of the factor vector and share no weights or activations. Their outputs $f_A=g_A(A)$, $f_B=g_B(B)$, $f_C=g_C(C)$ are therefore data-independent and can be computed concurrently on three lanes. All coupling is deferred to a single join point: the fused expert $f_F=\mathrm{fuse}([f_A,f_B,f_C])$, the gate $\alpha=\mathrm{softmax}(W_g[f_A,f_B,f_C]/\tau)$, the gated combination $b=\sum_m \alpha_m\, \mathrm{proj}_m$, and the query decoder all execute after a barrier. This is a map (three branches) followed by a reduce (fuse/gate/decode), not a set of independent end-to-end pipelines.

\textbf{Work--span bound for the S-AFF stage.}
Let $w_A,w_B,w_C$ be the per-frame cost of the three encoders and $w_J$ the cost of the joined fuse/gate/decode stage. Sequential execution costs $T_\text{seq}=w_A+w_B+w_C+w_J$; ideal three-lane execution costs $T_\text{par}=\max(w_A,w_B,w_C)+w_J$. The attainable speedup of the S-AFF stage is thus bounded by $(w_A+w_B+w_C)/\max(w_A,w_B,w_C)\le 3$, and in practice is smaller because the subcarrier branch (two 1D-convolution blocks) dominates $w_B\gg w_A,w_C$; the link and packet branches are shallow dense/1D-conv maps. Branch parallelism is therefore a modest constant-factor win \emph{on the S-AFF stage only}.

\textbf{The stage that dominates is CP, so pipelining is the stronger lever.}
The per-frame cost is $T_\text{CP}+T_\text{S\text{-}AFF}$, and $T_\text{CP}$ (iterative ALS) dominates $T_\text{S\text{-}AFF}$. Because the two stages are sequential \emph{within} a frame but independent \emph{across} frames, we double-buffer them: while stage~1 factorizes frame $t{+}1$, stage~2 runs S-AFF on frame $t$ (Alg.~\ref{alg:pipeline}). Steady-state throughput becomes $1/\max(T_\text{CP},T_\text{S\text{-}AFF})$ instead of $1/(T_\text{CP}+T_\text{S\text{-}AFF})$. Since $T_\text{CP}\gg T_\text{S\text{-}AFF}$, S-AFF is almost fully hidden behind the next frame's factorization: the correct conclusion is that intra-S-AFF branch parallelism matters little \emph{unless} CP is separately accelerated or offloaded, at which point S-AFF re-enters the critical path and Alg.~\ref{alg:forkjoin} becomes worthwhile. In this draft we use fixed rank profiles and adapt the CP iteration budget, not runtime rank switching.

\textbf{Heterogeneous mapping and data parallelism.}
The conv-heavy subcarrier branch maps well to an NPU/DSP while the dense link and packet branches run on CPU, allowing the branches to overlap on distinct units at no extra energy. Within CP, the matricized tensor-times-Khatri-Rao products are data-parallel across the $R$ components and across the three physical links (a $3\times3$ antenna array in Person-in-WiFi~3D), and the $Q$ person queries in the decoder are already a batched matmul with coupling confined to one small attention mixer.

\begin{algorithm}[t]
\caption{Fork--join S-AFF inference (per frame)}
\label{alg:forkjoin}
\begin{algorithmic}[1]
\Require CP factors $A,B,C$ at rank $R$
\State \textbf{parallel do} \Comment{three independent lanes}
\State \quad lane 1: $f_A \gets g_A(A)$
\State \quad lane 2: $f_B \gets g_B(B)$ \Comment{heaviest; place on NPU/DSP}
\State \quad lane 3: $f_C \gets g_C(C)$
\State \textbf{barrier}
\State $h \gets \mathrm{normalize}([f_A,f_B,f_C])$; \quad $f_F \gets \mathrm{fuse}(h)$
\State $\alpha \gets \mathrm{softmax}(W_g h/\tau)$; \quad $b \gets \sum_m \alpha_m \mathrm{proj}_m(f_m)$
\State $\{$poses, conf, count$\} \gets \mathrm{QueryDecoder}(b)$
\State \Return top-$k$ poses by confidence
\end{algorithmic}
\end{algorithm}

\begin{algorithm}[t]
\caption{Pipelined streaming with fixed rank profile}
\label{alg:pipeline}
\begin{algorithmic}[1]
\State double buffer $\mathcal{B}_0,\mathcal{B}_1$ for factor outputs
\For{each frame $t$}
  \State $R \gets$ configured deployment profile; $I_t \gets$ iteration budget
  \State \textbf{concurrently}:
  \State \quad \textbf{stage 1} (unit U$_1$): $\mathcal{B}_{t\bmod2} \gets \mathrm{CP\text{-}ALS}(\text{CSI}_t, R, I_t)$
  \State \quad \textbf{stage 2} (unit U$_2$): pose$_{t-1} \gets$ Alg.~\ref{alg:forkjoin}$(\mathcal{B}_{(t-1)\bmod2})$
  \State \textbf{barrier}; emit pose$_{t-1}$
\EndFor
\end{algorithmic}
\end{algorithm}

\textbf{Status of the claim.}
The independence above is a structural property of the architecture and is exact. We have measured its consequences on a Qualcomm aarch64 edge device (six cores, \texttt{5.4.233-perf} kernel) and they confirm the honest reading. A fork--join of the ONNX encoder branches is \emph{slower} than the single-thread sequential path ($0.70\times$). The S-AFF ONNX head runs in $0.18$--$0.75$\,ms single-thread and is \emph{slower} with six threads ($0.26$--$1.08$\,ms). CP-ALS thread scaling is null: six-thread extraction matches single-thread within measurement noise (Table~\ref{tab:ondevice}). Coarse-grain concurrency of the two independent CP streams (amplitude and phase) also fails: Python thread execution collapses under the GIL to $0.05\times$, and even persistent multiprocess workers reach only $0.85\times$, because the inter-process round-trip ($\sim$6.6\,ms for two jobs) is comparable to the $\sim$7\,ms per-stream compute. Across every granularity we tested---sub-millisecond neural branches and millisecond-scale CP streams---the parallelization overhead meets or exceeds the per-task work. We therefore do not claim a parallel speedup, because at batch~1 on this hardware there is none to claim; the deployable result is that a single core is already sufficient, and the fork--join/pipeline structure is a reserve lever for the case where CP is offloaded to a DSP/NPU.
...
\begin{table}[t]
\centering
\caption{Full-MM-Fi decomposition/regressor ablation. All methods use the same protocol-3 train/test split with 224,532 training frames and 48,114 test frames.}
\label{tab:decomp_regressor}
\begin{tabular}{lrrrr}
\toprule
Method & Params & MPJPE & PCK$_{30}$ & PCK$_{20}$ \\
\midrule
Mean pose & 0 & 0.246 & 46.92 & 27.45 \\
PCA + MLP & 111.9K & \textbf{0.187} & \textbf{68.91} & \textbf{53.53} \\
Matrix NMF + MLP & 111.9K & 0.257 & 50.68 & 32.85 \\
Tucker + MLP & 220.7K & 0.198 & 65.97 & 49.64 \\
CP + MLP & 209.2K & 0.191 & 67.99 & 52.09 \\
CP + S-AFF & 64.9K & 0.197 & 66.90 & 50.30 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:decomp_regressor} answers why we use CP rather than only reporting CP variants. PCA+MLP is the strongest pure predictor in this ablation, but it produces an unstructured projection that does not expose link, subcarrier, and packet factors. CP+MLP is within 1.44 PCK$_{20}$ points of PCA+MLP while preserving the physical CSI modes. CP+S-AFF gives a deployment-oriented point: under the shared ablation training protocol, it sacrifices 1.79 PCK$_{20}$ points relative to CP+MLP but reduces regressor parameters by 3.2$\times$ and preserves branch structure for selective routing. The tuned S-AFF run in Table~\ref{tab:main} reaches 50.80\% PCK$_{20}$. Matrix NMF and Tucker underperform CP under matched neural regressors. Thus, the reason to choose CP is not that it always maximizes raw accuracy; it is the best basis for an interpretable and hardware-shaped fusion architecture.
...
SwiftPose-Fi-L & 6 & 10--15 & Higher & Edge GPU \\
SwiftPose-Fi-XL & 8 & 10--15 & Highest & Unloaded edge \\
\bottomrule
\end{tabular}
\end{table}

The current experiments evaluate the middle profile, SwiftPose-Fi-M, because it balances accuracy and cost. The profiles are useful deployment targets, but we do not claim a validated runtime rank selector in this draft: a nested-anytime rank model reduced Person-in-WiFi 3D accuracy too much relative to the dedicated dual-CP model (Sec.~\ref{sec:rank_negative}). We therefore keep rank as a static deployment profile and focus the hardware contribution on measured latency and deadline-aware CP iteration preemption.

\subsection{On-Device Cost Structure (Qualcomm aarch64)}
\label{sec:ondevice}

We measured the two pipeline stages on a Qualcomm aarch64 edge device (six cores, \texttt{5.4.233-perf} kernel, Python~3.8.10) at batch~1, the frame-level deployment regime. Table~\ref{tab:ondevice} reports per-frame CP-ALS latency across rank and iteration count on a single core. Three facts stand out. First, CP-ALS latency is monotone and near-linear in both rank and iterations, so both are clean runtime latency dials: the rank-4/10-iteration MM-Fi profile costs $2.24$\,ms (about $446$ frames/s for extraction alone), and the rank-24/10-iteration profile used for dual-stream Person-in-WiFi~3D costs $7.44$\,ms per stream. Second, the S-AFF predictor is negligible beside it: the exported dual-CP S-AFF ONNX graph runs in $0.18$--$0.75$\,ms single-thread across the small/medium/large profiles, one to two orders of magnitude below CP, so CP is more than $90\%$ of end-to-end latency and is the only kernel worth optimizing. Third, thread scaling is null: six-thread CP-ALS matches single-thread within measurement noise, and the S-AFF ONNX head is actually \emph{slower} with six threads ($0.26$--$1.08$\,ms) than with one. On this device, a single core running sequentially is both the simplest and the fastest option at batch~1, which is why the validated runtime lever is deadline-aware CP iteration preemption rather than multicore parallelism.

\begin{table}[t]
\centering
\caption{Measured per-frame CP-ALS latency on the Qualcomm aarch64 device (single core, batch~1, median ms). Six-thread results match single-thread within noise.}
\label{tab:ondevice}
\begin{tabular}{lrrr}
\toprule
Rank $R$ & 5 iters & 10 iters & 35 iters \\
\midrule
2  & 0.90 & 1.60 & 5.08 \\
4  & 1.22 & 2.24 & 7.33 \\
6  & 1.41 & 2.61 & 8.59 \\
...
\subsection{Why We Do Not Claim Runtime Rank Adaptation}
\label{sec:rank_negative}

CP rank is an attractive deployment knob because latency grows monotonically with rank (Table~\ref{tab:ondevice}). We tested whether this could become a single-model runtime controller by training a nested-anytime dual-CP model that accepts rank prefixes. The result was not strong enough for the paper: after fixing the training protocol and running a 160-epoch sandwich-rule model on Person-in-WiFi~3D, the full-rank prefix reached only 121.05\,mm MPJPE, substantially worse than the dedicated rank-24 dual-CP model's 83.82\,mm, and the rank ladder was not monotonic (120.58/125.40/126.06/121.05\,mm for ranks 4/8/16/24). We therefore de-scope proactive rank adaptation from the claimed contribution. Rank remains useful for choosing a static deployment profile, but the validated runtime mechanism below adapts CP \emph{iterations} while preserving the dedicated model.

\subsection{Deadline-Preemptible Anytime Runtime}
\label{sec:preempt}

We adapt at a finer, retraining-free granularity. The CP-MU solver is itself an anytime algorithm over its \emph{iterations}: every multiplicative-update sweep yields valid factors, so the loop can be stopped at any point. We therefore run the \emph{dedicated} full-accuracy model and preempt only its CP iteration count to meet a latency deadline, leaving CP and S-AFF otherwise unchanged.

The runtime executes on one core (justified by Sec.~\ref{sec:parallel}). For each frame it reserves headroom for the S-AFF head, splits the remaining budget across the amplitude and phase CP streams, and stops each stream's ALS loop \emph{predictively}---before any sweep whose estimated duration (from the previous sweep) would overshoot the budget (Algorithm~\ref{alg:preempt}).

\begin{algorithm}[t]
\caption{Deadline-preemptible anytime frame inference}
\label{alg:preempt}
\begin{algorithmic}[1]
\Require deadline $D$, measured ONNX head cost $\hat{o}$, margin $m$, max iters $I$
\State $t_0 \gets \mathrm{now}$;\quad $B \gets D - \hat{o} - mD$ \Comment{CP budget after head reservation}
\For{stream $s \in \{\text{amp},\text{phase}\}$}
  \State $t_\text{stop} \gets t_0 + w_s B$ \Comment{split budget across the two streams}
  \State run CP-MU sweeps up to $I$; \textbf{before} each sweep, \textbf{if} $\mathrm{now}+\hat{\Delta} \ge t_\text{stop}$ \textbf{then stop} \Comment{$\hat{\Delta}$: last sweep time}
\EndFor
\State $\hat{y} \gets \mathrm{S\text{-}AFF}_\text{ONNX}(\text{factors}_\text{amp},\text{factors}_\text{phase})$
\State \Return $\hat{y}$
\end{algorithmic}
\end{algorithm}
...
\begin{table}[t]
\centering
\caption{On-device deadline-preemptible runtime (Qualcomm aarch64, dual-CP rank~24 + real S-AFF ONNX head, $D=35$\,ms, 50 frames/policy, single core). Iterations are the amp+phase total. ``Loaded'' runs three background CPU burners.}
\label{tab:preempt}
\begin{tabular}{llrrr}
\toprule
Condition & Policy & Median (ms) & Miss \% & Iters \\
\midrule
Idle   & fixed-hi (35) & 46.5 & 100 & 70.0 \\
Idle   & fixed-lo (8)  & 12.2 & 0   & 16.0 \\
Idle   & \textbf{preemptive} & 29.9 & \textbf{0} & \textbf{43.7} \\
Loaded & fixed-hi (35) & 51.6 & 100 & 70.0 \\
Loaded & fixed-lo (8)  & 13.3 & 0   & 16.0 \\
Loaded & \textbf{preemptive} & 29.6 & \textbf{0} & \textbf{39.0} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:preempt} reports the measured behavior on the device with the real \texttt{dual\_saff\_full} ONNX head, a 35\,ms deadline, under idle and real background CPU load. A fixed high iteration count gives the most refinement but misses the deadline on every frame; a fixed low count meets the deadline but wastes two-thirds of the budget; the preemptive runtime meets the deadline on every frame while delivering $2.4$--$2.7\times$ more iterations than the fixed-low policy, and it \emph{adapts down} under contention ($43.7\to39.0$ total iterations) to preserve the deadline.

\textbf{What is and is not yet established.} This validates the runtime's \emph{timing} behavior---hard deadline adherence with maximal in-budget refinement---on real hardware, single core, with no retraining and CP/S-AFF unchanged. It does not by itself establish that fewer iterations preserve pose \emph{accuracy}; that requires the dedicated model's MPJPE at CP iterations $\{3,5,10,20,35\}$, which is the pending measurement. The design premise is that the dedicated model, evaluated at the $\sim$20 iterations/stream the runtime delivers within 35\,ms, remains near its 88\,mm operating point; if instead accuracy degrades sharply below full iterations, preemption trades accuracy like rank adaptation does, and we will report that outcome honestly.
